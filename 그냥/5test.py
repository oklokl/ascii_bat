...
yes_button = Button(frame, text='YES', width=15, height=3, font=button_font,
                    command=lambda: toggle_button(yes_button))  # 버튼 크기 수정
yes_button.grid(row=0, column=0, padx=(100, 0), pady=(50, 50))  # 패딩 추가

no_button = Button(frame, text='NO', width=15, height=3, font=button_font,
                   command=lambda: toggle_button(no_button))  # 버튼 크기 수정
no_button.grid(row=0, column=1, padx=(100, 0), pady=(50, 50))  # 패딩 추가
no_button.config(state='disabled')

frame.pack()
root.geometry('1000x300')  # 창 크기 수정 (.geometry를 이곳으로 이동)
root.mainloop()
